...
def pipeline_olho_vivo():
    ...
    def extrair_posicoes_sptrans():
        ...
        # Definição de data e timestamp
        agora = pendulum.now("America/Sao_Paulo")
        data_particao = agora.strftime("%Y-%m-%d")      # Ex: '2026-08-28' (formato comum para particionamento)
        ts = agora.strftime("%Y%m%d_%H%M%S")

        # Cria pasta diária para JSON
        output_dir = Path("/opt/airflow/data/raw/POSICAO") / f"data={data_particao}"
        ...
        output_dir_parquet = Path("/opt/airflow/data/raw/POSICAO_parquet") / f"data={data_particao}"
